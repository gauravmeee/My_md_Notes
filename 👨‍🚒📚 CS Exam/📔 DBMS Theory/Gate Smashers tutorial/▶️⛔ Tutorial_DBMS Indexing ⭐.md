...
Data is Stored inside Secondary Memory
For Example Records are stored in Hard disk like this
...
**Think it as Like** -> 
- Index in Front of a Book -> Primary Index
	One entry → one data block (often sparse)
	tells you _where chapters start_ → direct navigation
	faster, structured access

- Index in Back of a Book -> Secondary Page
	One key → **multiple references** to data rows (dense)
	tells you _where a word appears_ → many scattered pages
	flexible, but needs extra lookup
