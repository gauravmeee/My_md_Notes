...
- Common choice: $c_1 = 0, c_2 = 1$ → $h(k, i) = (h(k) + i^2) \bmod m$
...
- Uses two hash functions: $h_1(k)$ and $h_2(k)$
- **Critical requirement:** $h_2(k)$ must never be 0
- **Best practice:** $h_2(k)$ relatively prime to m

- **Common choices:**
	- $h_1(k) = k \bmod m$
	- $h_2(k) = 1 + (k \bmod m')$ where $m' < m$
	- If m is prime: $h_2(k) = 1 + (k \bmod (m-1))$
...
10. **Double hashing requirements:** $h_2(k) \neq 0$, relatively prime to m
...
4. **Double Hashing Independence:**
    ...
    - $h_2(k)$ must be independent of $h_1(k)$
