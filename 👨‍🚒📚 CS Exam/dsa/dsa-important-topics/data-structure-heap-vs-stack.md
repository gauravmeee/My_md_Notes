...
2. **Size**:
   * The heap can <mark>grow and shrink as neededn</mark> depending on the memory requirements of the application. It is generally <mark>larger than the stackn</mark>
3. **Lifetime**:
...
2. **Automatic Memory Management**:
   * Memory <mark>allocation and deallocation for stack variables are handled automaticallyn</mark> When a function is called, its local variables are pushed onto the stack, and when the function exits, these variables are popped off the stack.
3. **Size**:
   * The stack typically has a <mark>fixed sizen</mark> which is <mark>determined when a program startsn</mark> Exceeding this size <mark>can lead to a **stack overflow**n</mark>
4. **Lifetime**:
   * Variables on the stack <mark>exist only within the scope of the function</mark> in which they are declared. Once the function exits, these variables are destroyed.
5. **Access Speed**:
