...
**Problem** Process is divided according to the hole sizes, but The holes are dynamically created, whenever a process try to enter into RAM, the CPU need to analyze the no. of holes and there size and then divide the process according to it. But dividing a process is time/Costly Method

**Solution** So a Processes is divided before its gets the Memory called **pages.** We make and keep partition of Process already in Secondary Memory that is page We already divide Main memory in Partitions i.e. each slots called **Frames**
