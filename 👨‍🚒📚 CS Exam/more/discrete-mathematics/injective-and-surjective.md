...
- $|A| >= |B|$ (`m >= n`) **Reason**: Surjective means $f(A) = B$ So number of **distinct outputs** = `n`\
  But distinct outputs ≤ number of inputs = m\
  \=> `n ≤ m`

**2. if f is Injective (One-One)**

- $|A| <= |B|$ (`m <= n`) **Reason:** Injective means all inputs give distinct outputs\
...
- $|A| = |B|$ (`m = n`) **Reason:** From surjective => `m >= n`\
...
- **Proof:**
  - Suppose $g(x\_1) = g(x\_2)$ for $x\_1, x\_2 \in A$.
  - Apply $f$ to both sides: $f(g(x\_1)) = f(g(x\_2) \implies (f \circ g)(x\_1) = (f \circ g)(x\_2)$
  - Since $(f \circ g)$ is injective, we must have $x\_1 = x\_2$.
...
- **Proof:**
  - Suppose $(f \circ g)(x\_1) = (f \circ g)(x\_2)$.
  - Then $f(g(x\_1)) = f(g(x\_2))$
  - Since (f) is injective, $g(x\_1) = g(x\_2)$
  - Since (g) is injective, $x\_1 = x\_2$.
